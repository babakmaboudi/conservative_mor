...
Consider a 2-dimensional compressible Euler equation \eqref{eq:3.18} in a periodic square box $[0,1]^2$. Unlike the incompressible example in \Cref{sec:res.1} a centered finite difference scheme of fourth order are used to discretize \eqref{eq:3.18}. The physical domain is discretized into a grid of $256\times 256$ nodes.
...
\Cref{fig:error_decay_KH} illustrates that the accuracy of the method is consistently increase as higher number of POD basis modes are considered. Furthermore, the skew-symmetric form preserves the accuracy over the period of time integration. It is observed from \Cref{fig:snap_solution_KH} that all features of the flow are correctly represented in the reduced system, even with a low number of basis vectors.
...
Conservation mass, momentum and energy is presented in \Cref{fig:conservation_DEIM}. The accuracy of the method in approximating these invariants is enhanced when the size of the basis is increased. Furthermore, \Cref{energy_error_KH} shows how the kinetic energy associated with the reduced system mimic the kinetic energy of the high-fidelity system. This ensures the correct evolution of kinetic energy, and thus, the internal energy.

This helps with the correct distribution of the energy and ensures 
\begin{figure}
  \centering
  \begin{subfigure}[]{0.48\linewidth}
  \begin{tikzpicture}[scale=0.55]
...
In this section we study the 1-dimensional compressible Euler problem, \eqref{eq:3.18} without viscous terms, with a steady state discontinuous solution. This is in preparation for \Cref{sec:5.res.4}, where development and propagation of shock waves is expected. Here we asses how the skew-symmetric form for \eqref{eq:3.18} can recover moving discontinuities at the level of reduced system. Consider a periodic boundary conditions on $\Omega = [0,1]$ with the initial condition`
\begin{equation*}
\begin{cases}
