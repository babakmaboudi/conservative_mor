...
\Cref{compare_different_models} illustrates the evolution of the kinetic energy for the advective, divergence, and the skew-symmetric form of the high-fidelity system. It is observed that only the skew-symmetric form preserves the kinetic energy, confirming the results in \Cref{sec:skew.2}.
...
\caption{(\protect\subref{fig:approx_error_incompressible_Euler}) Evolution of 2-norm error in velocity, between the high-fidelity system and the reduced system. (\protect\subref{fig:energy_error_incompressible_Euler}) Conservation of the kinetic energy.}
\end{figure}
...
The time marching method adopted for the discretization of the time derivatives for the complete and reduced models is the scheme proposed by Morinishi \eqref{eq:3.27}. The final time considered is $T=1s$ with a time step $\Delta t =5 \cdot 10^{-4}$. 

In \Cref{fig:error_decay_KH} we show that the solution to the skew-symmetric reduced models are stable and the accuracy in the approximation of the high fidelity solution increases with the dimension of the set of basis functions considered. From \Cref{fig:snap_solution_KH}, where snapshots of the high fidelity and reduced solutions at different time steps are shown, we notice that even in the less accurate case considered,  the reduced solution is not easily distinguishable from the high fidelity solution.
\begin{figure}
\centering
...
In \Cref{fig:conservation_DEIM}  we show that the considered invariants of the system converges to the related invariatns of the high fidelity solution.
\begin{figure}
  \centering
  \begin{subfigure}[]{0.48\linewidth}
  \begin{tikzpicture}[scale=0.55]
...
One of the most challenging aspects of model order reduction of combustion problems is the development 
and propagation of shocks in the solution, starting from the simple steady state scenario. Before proceeding with the reduction of  such a problem, we focus on a much simpler case to assess the capability of the skew-symmetric formulation to handle moving discontinuities in combination to the reduced basis approach. We consider the 1D compressible Euler equation with periodic boundaries on the domain $[0,1]$. The initial conditions are given by
\begin{equation*}
\begin{cases}
...
\end{cases}
\end{equation*}
The domain is discretized using $2000$ nodes and spatial derivatives are approximated using a second order centered finite differences scheme. We simulate $T=0.3$s of the dynamic of the system, using Morinishi time integrator. Artificial viscosity is added, only to the pressure equation, with a constant diffusion coefficient equals to $\mu = 0.5 \cdot 10^{-4}$.
First we consider a comparison between different formulations of the compressible Euler equation. In   \Cref{fig:energy_pres_1D}, we show the behaviour of the conserved quantities mass, total momentum and total energy, for the reduced solution of the skew-symmetric, divergence and advective formulations, while in \Cref{fig:snaps_1D_Euler} we show snapshots of the solution. The skew-symmetric formulation is the only one preserving these invariants, while other formulations are unstable. Moreover we notice that for unstable formulations, the more basis functions we use, the sooner the reduced solutions blow up. On the contrary,  for the stable formulation, increasing the number of basis leads to a more accurate representation, as shown in \Cref{error_behaviour_1D}. However, the introduction of the artificial diffusion  slightly affects the conservation properties of the algorithms, as can be seen in   \Cref{conservation_1D_focus}. 

For this problem we estimate the effect of the DEIM procedure on the accuracy and on the conservation of invariants for the solution of the reduced problem.  First we notice, from \Cref{fig:singular_values_decay_1D}, that the space spanned by the snapshots of nonlinearity is larger than the space spanned by the solution snapshots. Hence, we expect that to reach the same accuracy of standard POD with the DEIM approach, more basis will be required.
This conclusion is numerically justified by a direct comparison of the approximation error (\Cref{error_DEIM}) and of the total energy of the system (\Cref{energy_DEIM}) at the last time step. The number of POD basis has been fixed to $204$ while the number of DEIM basis adopted for the simulation varies in the interval  $[204,500]$. Even though the interpolatory procedure have an impact on the quality of the reduction, the reduced solution remains an accurate representation of the high fidelity solution, preserving stability and the boundedness of the conserved quantities. By increasing the number of DEIM basis,  the error is dominated by the POD approximation error.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{tikzpicture}[scale=0.5452]
    ...
    \end{semilogyaxis}
\end{tikzpicture}
\caption{Singular values decay of the snapshot matrix related to POD and DEim algorithms for the 1D compressible Euler problem.}
